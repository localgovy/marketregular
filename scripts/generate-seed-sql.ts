...
if (launchVendors.length) {
  lines.push(
    "insert into public.vendors (id, slug, name, about, website, instagram, tiktok, facebook, phone, email, tags, status) values",
  );
  lines.push(
    launchVendors
      .map(
        (v) =>
          `  (${sqlStr(v.id)}, ${sqlStr(v.slug)}, ${sqlStr(v.name)}, ${sqlStr(v.about)}, ${sqlStr(v.website)}, ${sqlStr(v.instagram ?? null)}, ${sqlStr(v.tiktok ?? null)}, ${sqlStr(v.facebook ?? null)}, ${sqlStr(v.phone)}, ${sqlStr(v.email ?? null)}, ${sqlArr(v.tags)}, 'published')`,
      )
